    ...
    public class TokenStorageService
    {
        ...
        private async Task InitializeDefaultTokenAsync()
        {
            try
            {
                ...
                var autoLogin = _configuration.GetValue<bool>("Development:AutoLogin", true);
                if (!autoLogin)
                {
                ...
                var defaultUsername = _configuration["Development:DefaultUsername"] ?? "admin";
                var defaultPassword = _configuration["Development:DefaultPassword"] ?? "admin123";

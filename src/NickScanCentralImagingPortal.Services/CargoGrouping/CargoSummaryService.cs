...
namespace NickScanCentralImagingPortal.Services.CargoGrouping
{
    ...
    /// </summary>
    public class CargoSummaryService : ICargoSummaryService
    {
        ...
        public Task<CargoSummaryDto> GenerateSummaryAsync(CargoGroupDto cargoGroup)
        {
            ...
            {
                _logger.LogError(ex, "Error generating cargo summary for group {GroupIdentifier}", cargoGroup.GroupIdentifier);
        ...
        private void ExtractGoodsDescription(CargoSummaryDto summary, List<ICUMSDataRecordDto> records, List<BOEDetailDto> boeDetails)
        {
            ...
            if (descriptions.Any())
            {
                // If multiple descriptions, try to find common themes or combine intelligently
                var uniqueDescriptions = descriptions.Distinct().ToList();
                if (uniqueDescriptions.Count == 1)
                {
                    summary.GoodsDescription = uniqueDescriptions.First();
                }
                else if (uniqueDescriptions.Count <= 3)
                {
                ...
                {
                    // Too many descriptions - use first and indicate more
                    summary.GoodsDescription = $"{uniqueDescriptions.First()} (and {uniqueDescriptions.Count - 1} other item type(s))";
                }
            }
        }
        ...
        private void ExtractHSCodes(CargoSummaryDto summary, List<ICUMSDataRecordDto> records)
        {
            ...
            {
                // Extract HS code pattern (e.g., "8541.40" or "854140")
        ...
        private void ExtractQuantities(CargoSummaryDto summary, List<ICUMSDataRecordDto> records)
        {
            ...
            {
                // Parse quantity (e.g., "1250.00 PCS" or "1250")
        ...
        private void ExtractWeight(CargoSummaryDto summary, List<ICUMSDataRecordDto> records)
        {
            ...
            {
                // Parse weight (e.g., "2450.00 kg" or "2450 kg")
        ...
        private void ExtractFOBValue(CargoSummaryDto summary, List<ICUMSDataRecordDto> records)
        {
            ...
            {
                // Parse FOB value (e.g., "$45,200.00 USD" or "45200 USD")
        ...
        {
            // Count distinct items by looking for item prefixes or item numbers
            ...
            if (itemFields.Any())
            {
                // Extract item numbers from field names (e.g., "Item 1: HS Code" -> 1)
                ...
                {
                    // Fallback: count unique HS codes or descriptions
            ...
            {
                summary.LineItemCount = 1; // Default to 1 if we can't determine
            }
        }
